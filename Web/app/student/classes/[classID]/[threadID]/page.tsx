...
const ThreadPage = ({
  ...
  useEffect(() => {
    const fetchThreadDetails = async () => {
      try {
        const response = await fetch(
          `/api/student/classes/${params.classID}/${params.threadID}`
        );
    ...
    fetchThreadDetails();
  }, [params.classID, params.threadID]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `/api/student/classes/${params.classID}/${params.threadID}/post-reply`,
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        const data = await updatedThread.json();
        setThreadData(data);
      } else {
  ...
  return (
    <div className="px-4 sm:px-0 py-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{threadData.title}</CardTitle>
          <div className="flex items-center text-gray-500 text-sm">
            <span>{threadData.main_post.created_by}</span>
          </div>
        </CardHeader>
        <CardContent>
          <p className="mb-4">{threadData.main_post.description}</p>
